...
#[tauri::command]
fn reorder_pages(path: String, pages: Vec<u32>, insert_before: u32) -> Result<(), String> {
    let mut doc = Document::load(&path).map_err(|e| format!("Failed to load PDF: {}", e))?;
    let total = doc.get_pages().len() as u32;
    ...
    let items: Vec<Object> = indices.iter().map(|idx| {
        extracted.iter().find(|(i, _)| i == idx).unwrap().1.clone()
    }).collect();
...
#[tauri::command]
fn set_page_order(path: String, order: Vec<u32>) -> Result<(), String> {
    let mut doc = Document::load(&path).map_err(|e| format!("Failed to load PDF: {}", e))?;
    let total = doc.get_pages().len() as u32;
